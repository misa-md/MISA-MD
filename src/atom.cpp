...
atom::atom(DomainDecomposition *domain, double latticeconst,
    ...
    */
    ...
    nghostx = _domain->getSubBoxLatticeSize(0) + 2 * 2 * ceil(domain->getMeasuredGhostLength(0) / _latticeconst);
    nghosty = _domain->getSubBoxLatticeSize(1) + 2 * ceil(domain->getMeasuredGhostLength(1) / _latticeconst);
    nghostz = _domain->getSubBoxLatticeSize(2) + 2 * ceil(domain->getMeasuredGhostLength(2) / _latticeconst);
...
//    printf("number:%d, %d, %d, %d, %d\n", numberoflattice, _domain->getSubBoxLatticeSize(0), _domain->getSubBoxLatticeSize(1), _domain->getSubBoxLatticeSize(2), _domain->getSubBoxLatticeSize(0)*_domain->getSubBoxLatticeSize(1)*_domain->getSubBoxLatticeSize(2));
    ...
    if (isAccelerateSupport()) {
        accelerateInit(lolocalx, lolocaly, lolocalz, _domain->getSubBoxLatticeSize(0), _domain->getSubBoxLatticeSize(1),
...
void atom::addatom(unsigned long id, double rx, double ry, double rz, double vx, double vy, double vz) {
    int i;
    if ((rx >= _domain->getMeasuredSubBoxLowerBounding(0)) && (rx < _domain->getMeasuredSubBoxUpperBounding(0)) &&
        (ry >= _domain->getMeasuredSubBoxLowerBounding(1)) && (ry < _domain->getMeasuredSubBoxUpperBounding(1)) &&
        (rz >= _domain->getMeasuredSubBoxLowerBounding(2)) && (rz < _domain->getMeasuredSubBoxUpperBounding(2))) {
        int lattice[3];
...
void atom::getatomx(int direction, vector<vector<int> > &sendlist) {
    ...
    if (direction == 0) {
        ...
        //找到要发送到邻居进程的区域
        int xstart = lolocalx - loghostx + _domain->getSubBoxLatticeSize(0) - ((_cutlattice) * 2);
        int ystart = lolocaly - loghosty;
        int zstart = lolocalz - loghostz;
        int xstop = lolocalx - loghostx + _domain->getSubBoxLatticeSize(0);
        int ystop = ystart + _domain->getSubBoxLatticeSize(1);
        int zstop = zstart + _domain->getSubBoxLatticeSize(2);
...
void atom::getatomy(int direction, vector<vector<int> > &sendlist) {
    ...
    if (direction == 0) {
        ...
        //找到要发送到邻居进程的区域
        int xstart = 0;
        int ystart = lolocaly - loghosty + _domain->getSubBoxLatticeSize(1) - (_cutlattice);
        int zstart = lolocalz - loghostz;
        int xstop = nghostx;
        int ystop = lolocaly - loghosty + _domain->getSubBoxLatticeSize(1);
        int zstop = zstart + _domain->getSubBoxLatticeSize(2);
...
void atom::getatomz(int direction, vector<vector<int> > &sendlist) {
    ...
    if (direction == 0) {
        ...
        int xstart = 0;
        int ystart = 0;
        int zstart = lolocalz - loghostz + _domain->getSubBoxLatticeSize(2) - (_cutlattice);
        int xstop = nghostx;
        int ystop = nghosty;
        int zstop = lolocalz - loghostz + _domain->getSubBoxLatticeSize(2);
...
void atom::unpack_recvfirst(int d, int direction, int n, LatParticleData *buf, vector<vector<int> > &recvlist) {
    ...
    if (d == 0) {
        if (direction == 0) {
            xstart = lolocalx - loghostx + _domain->getSubBoxLatticeSize(0);
            xstop = nghostx;
        ...
        if (direction == 0) {
            xstart = 0;
            xstop = nghostx;
            ystart = lolocaly - loghosty + _domain->getSubBoxLatticeSize(1);
            ystop = nghosty;
            zstart = lolocalz - loghostz;
            zstop = zstart + _domain->getSubBoxLatticeSize(2);
            for (int k = zstart; k < zstop; k++) {
        ...
        if (direction == 0) {
            ...
            ystart = 0;
            ystop = nghosty;
            zstart = lolocalz - loghostz + _domain->getSubBoxLatticeSize(2);
            zstop = nghostz;
            for (int k = zstart; k < zstop; k++) {
                for (int j = ystart; j < ystop; j++) {
                    for (int i = xstart; i < xstop; i++) {
...
void atom::setv(int lat[4], double collision_v[3]) {
    int kk;
    if ((lat[0] * 2) >= lolocalx && (lat[0] * 2) < (lolocalx + _domain->getSubBoxLatticeSize(0))
        && lat[1] >= lolocaly && lat[1] < (lolocaly + _domain->getSubBoxLatticeSize(1))
        && lat[2] >= lolocalz && lat[2] < (lolocalz + _domain->getSubBoxLatticeSize(2))) {
...
void atom::createphasespace(double factor, int box_x, int box_y, int box_z) {
    ...
    numbefore += (unsigned long) lolocaly * box_x * _domain->getSubBoxLatticeSize(2);
    numbefore += (unsigned long) lolocalx * _domain->getSubBoxLatticeSize(1) * _domain->getSubBoxLatticeSize(2);
