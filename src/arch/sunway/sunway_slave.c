...
__thread_local int bigblocknum;
...
void init(int *a){
	...
	bigblocknum = nlocalz - 128*block_size;
...
{   	if(subindex < Nvalue-bigblocknum){
		...
		return start+((natom/Nvalue)*subindex+subindex-(Nvalue-bigblocknum));
	}
}
...
int EndOfArea(int natom, int start,int subindex)
{
	if(subindex < Nvalue-bigblocknum){
		...
		return start+((natom/Nvalue)*(subindex+1)+(subindex+1)-(Nvalue-bigblocknum));
	}
}
