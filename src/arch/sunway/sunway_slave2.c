	...
	for(k = zOmp_start; k < zOmp_end; k++){
		for(j = ystart; j < nlocaly + ystart; j++){
			...
			for(m_step = 0; m_step < n_block; m_step++){
				...
				athread_get(PE_MODE, &x[kk*3], &_x[16*5*3], 16*5*3*8, &reply, 0, (nghostx-16)*3*8, 16*3*8);
				athread_get(PE_MODE, &rho[kk], &_rho[16*5], 16*5*8, &reply, 0, (nghostx-16)*8, 16*8);
				kk = IndexOf3DIndex( 8*(m_step%n_block), j-2, k+2);
				athread_get(PE_MODE, &x[kk*3], &_x[16*10*3], 16*5*3*8, &reply, 0, (nghostx-16)*3*8, 16*3*8);
					...
					if(xtemp != -100){
						for(l = 0; l < 68; l++){
							...
							if (dist2 < ( cutoffRadius * cutoffRadius )){
								...
								_spline[2] = ((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0;
								_spline[1] = 3.0*(_spline_[m+1]-_spline_[m]) - 2.0*_spline[2] - ((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0;
								_spline[0] = _spline[2] + ((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0 - 2.0*(_spline_[m+1]-_spline_[m]);
								//rtc(&stop);
	...
	for(k = zOmp_start; k < zOmp_end; k++){
		for(j = ystart; j < nlocaly + ystart; j++){
			...
			for(i = xstart; i < nlocalx + xstart; i++){
				...
				p = min(p, 1.0);
				//while(reply!=1);
				_spline[2] = (((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0) * invDx;
				_spline[1] = 2.0 * (3.0*(_spline_[m+1]-_spline_[m]) - 2.0*(((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0) - (((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0)) * invDx;
				_spline[0] = 3.0 * ((((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0) + (((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0) - 2.0*(_spline_[m+1]-_spline_[m])) * invDx;
	...
	for(k = zOmp_start; k < zOmp_end; k++){
		for(j = ystart; j < nlocaly + ystart; j++){
			for(m_step = 0; m_step < n_block; m_step++){
				...
				kk = IndexOf3DIndex( 8*(m_step%n_block), j-2, k+1);
				athread_get(PE_MODE, &x[kk*3], &_x[16*5*3], 16*5*3*8, &reply, 0, (nghostx-16)*3*8, 16*3*8);
				athread_get(PE_MODE, &f[kk*3], &_f[16*5*3], 16*5*3*8, &reply, 0, (nghostx-16)*3*8, 16*3*8);
				//athread_get(PE_MODE, &df[kk], &_df[16*5], 16*5*8, &reply, 0, (nghostx-16)*8, 16*8);
				kk = IndexOf3DIndex( 8*(m_step%n_block), j-2, k+2);
				athread_get(PE_MODE, &x[kk*3], &_x[16*10*3], 16*5*3*8, &reply, 0, (nghostx-16)*3*8, 16*3*8);
				athread_get(PE_MODE, &f[kk*3], &_f[16*10*3], 16*5*3*8, &reply, 0, (nghostx-16)*3*8, 16*3*8);
					...
					if(xtemp != -100){
						for(l = 0; l < 68; l++){
							...
							if (dist2 < ( cutoffRadius * cutoffRadius )){
								...
								_spline[5] = ((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0;
								_spline[4] = 3.0*(_spline_[m+1]-_spline_[m]) - 2.0*_spline[5] - ((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0;
                                                                _spline[3] = _spline[5] + ((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0 - 2.0*(_spline_[m+1]-_spline_[m]);
								_spline[2] = (((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0) * invDx;
                                				_spline[1] = 2.0 * (3.0*(_spline_[m+1]-_spline_[m]) - 2.0*(((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0) - (((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0)) * invDx;
                                				_spline[0] = 3.0 * ((((_spline_[m-2]-_spline_[m+2]) + 8.0*(_spline_[m+1]-_spline_[m-1])) / 12.0) + (((_spline_[m-1]-_spline_[m+3]) + 8.0*(_spline_[m+2]-_spline_[m])) / 12.0) - 2.0*(_spline_[m+1]-_spline_[m])) * invDx;
				...
				kk = IndexOf3DIndex( 8*(m_step%n_block), j-2, k+1);
				athread_put(PE_MODE, &_f[16*15], &f[kk*3], 16*15*8, &put_reply, (nghostx-16)*3*8, 16*3*8);
				kk = IndexOf3DIndex( 8*(m_step%n_block), j-2, k+2);
				athread_put(PE_MODE, &_f[16*10*3], &f[kk*3], 16*15*8, &put_reply, (nghostx-16)*3*8, 16*3*8);
				while(put_reply!=3);
			}
		}
	}
