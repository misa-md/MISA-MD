...
bool verify_offset_pairs(std::vector<std::pair<CutoffAtom, CutoffAtom>> &offset_pairs,
                         std::vector<NeiOffset> &nei_offsets, const double lattice_const) {
    //assert whether 2 atoms is in nei_index
    bool is_included = false;

    for (auto atomIt : offset_pairs) {
        for (auto it : nei_offsets) {
            if ((atomIt.first.coords[0] + it.x == atomIt.second.coords[0]) &&
                (atomIt.first.coords[1] + it.y == atomIt.second.coords[1]) &&
                (atomIt.first.coords[2] + it.z == atomIt.second.coords[2])) {
                is_included = true;
                break;
            }
        }
...
TEST(atom_cutoff_radius_included_test, lattice_included_test) {
    ...
    for (int l = 0; l < 512; l++) {
        ...
        const bool passed_odd = verify_offset_pairs(offset_pairs_odd, nei_index.nei_odd_offsets, lattice_const);
        EXPECT_TRUE(passed_odd);
        if (!passed_odd) {
            GTEST_FAIL();
        }

        const bool passed_even = verify_offset_pairs(offset_pairs_even, nei_index.nei_even_offsets, lattice_const);
